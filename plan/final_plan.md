...
- **의존성: 소스 벤더링(레포 커밋, submodule 아님).** 업스트림 드리프트 차단.
  - 방법: `git clone --branch <tag>` → `.git` 제거 → `third_party/<lib>/` 복사 → 커밋. 루트 `README.md` 벤더 표에 출처·버전·날짜·sha256 기록. 업데이트는 "다시 받아 교체 후 커밋"만.
...
`wu/agent`(+`wu/server` 골격) 생성. CMake(C++17, `CXX_STANDARD_REQUIRED ON`, `EXTENSIONS OFF`, ≥3.14, OS/arch 감지). `third_party/` 골격. `main.cpp` 은 config 체크+로그만 — 외부 lib 링크 없이 빌드 성공. `.gitignore`(build/, models/).
...
```
- 벤더링: `third_party/nlohmann/json.hpp`(v3.11.3), `third_party/llama.cpp`(commit 2969d6d, ggml 0.16.0). 루트 `README.md` 참조.
